...
public class MyPageInfoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 로그인한 멤버의 정보를 모두 얻어오기
		HttpSession session = req.getSession();
		
		// 세션에 저장된 로그인한 회원의 객체의 참조 주소를 복사 == 얕은 복사 
		Member loginMember = (Member)session.getAttribute("loginMember");
		int memberNo = loginMember.getMemberNo();
		
		// Member객체를 생성해서 update에 필요한 값을 세팅
		...
		try {
			...
			if(result > 0) {
				...
				// DB 데이터 수정 성공 시 
				// Session에 담긴 로그인 회원 정보도 같이 수정(데이터 동기화)
			...
			// 성공과 실패에 관계없이 메세지를 보내고 내 정보 페이지 재요청
			...
			String errorMessage = "회원정보 수정중 문제가 발생했습니다";
