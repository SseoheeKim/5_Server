...
public class LogoutFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// 로그인 상태를 검사하는 방법
		// == session 내에 loginMember가 null인지 아닌지 검사
		
		// -> Session 객체는 HttpServletRequest에서만 얻어올 수 있으므로 다운캐스팅 필요!
		HttpServletRequest req = (HttpServletRequest)request;
		HttpServletResponse resp = (HttpServletResponse)response; // 리다이렉트용
		
		HttpSession session = req.getSession();
		if(session.getAttribute("loginMember") != null) {
			session.setAttribute("message", "로그인 상태에서는 이용할 수 없는 서비스입니다." );
			resp.sendRedirect("/");
